...
module Where = Dune_rpc_impl.Where
module Scheduler = Dune_engine.Scheduler
module Server = Dune_rpc_server.Make (Csexp_rpc.Session)
module Action_exec = Dune_engine.Private.Action_exec
  ...
  let run_worker () =
    ...
    let pid =
      ...
      Spawn.spawn ~prog ~env
        ~argv:[ "dune"; "internal"; "action-runner"; "--root"; "."; name ]
        ()
      |> Pid.of_int
    in
...
let%expect_test "run an action runner and dispatch one job to it" =
  ...
  let config : Scheduler.Config.t =
    { concurrency = 2
    ; display = { Dune_engine.Display.status_line = false; verbosity = Quiet }
    ; stats = None
