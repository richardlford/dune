...
module Worker = struct
  let start ~name ~where =
    ...
    let private_menu : Client.proc list =
      ...
      [ Request Decl.ready
      ; Handle_request (Decl.exec, Action_exec.exec ~build_deps)
      ]
    in
    Client.client ~private_menu connection
      (Dune_rpc.Initialize.Request.create
         ~id:(Dune_rpc.Id.make (Sexp.Atom name)))
      ~f:(fun client ->
        let* request =
        ...
        | Error v ->
          User_error.raise
            [ Pp.textf
                "Server does not agree on the menu. Are you running the same \
                 dune binary for the worker?"
